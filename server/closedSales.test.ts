...
describe("closedSales.pendingPylonReview", () => {
  it("returns pending pylon sales with pylon-pending-review status", async () => {
    const { ctx } = createAuthContext();
    const caller = appRouter.createCaller(ctx);

    const result = await caller.closedSales.pendingPylonReview();

    expect(result).toBeDefined();
    expect(result.sales).toBeDefined();
    expect(Array.isArray(result.sales)).toBe(true);
    expect(typeof result.count).toBe("number");
    expect(result.count).toBe(result.sales.length);
  });
});

describe("closedSales.confirmPylonSale", () => {
  it("confirms a pylon sale and updates its status to contract-signed", async () => {
    const { ctx } = createAuthContext();
    const caller = appRouter.createCaller(ctx);

    // First create a sale manually with pylon-pending-review status via update
    const created = await caller.closedSales.create({
      customerName: "Pylon Review Test",
      customerPhone: "0433333333",
      installAddress: "55 Pylon St, Brisbane QLD 4000",
    ...
    // Set it to pylon-pending-review
    await caller.closedSales.update({
      id: created.id,
      data: { projectStatus: "pylon-pending-review" },
    });

    // Verify it appears in pending review
    const pending = await caller.closedSales.pendingPylonReview();
    const found = pending.sales.find((s) => s.id === created.id);
    expect(found).toBeDefined();
    expect(found!.customerName).toBe("Pylon Review Test");

    // Now confirm the sale with updated data
    const confirmed = await caller.closedSales.confirmPylonSale({
      id: created.id,
      data: {
        customerName: "Pylon Review Test - Confirmed",
        dealOwner: "George Fotopoulos",
        leadSource: "Pylon",
      },
    });
    ...
    expect(confirmed.projectStatus).toBe("contract-signed");
    expect(confirmed.customerName).toBe("Pylon Review Test - Confirmed");

    // Verify it no longer appears in pending review
    const pendingAfter = await caller.closedSales.pendingPylonReview();
    const notFound = pendingAfter.sales.find((s) => s.id === created.id);
