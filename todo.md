...
- [x] Fix: Solar Quotes notes (quarterly bill, system size) should write to column L (REQUESTED SUMMARIES) not column K (NOTES)
- [x] Backfill existing leads to move notes data from column K to column L
- [x] Add "Pre-Onsite Inspection & Deposit Confirmation" email template (generic, no reference number or deposit percentage)
- [x] Create Closed Sale entry for Gian Carlo Baldueza from Pylon signed PDF
- [x] Build automated Pylon signed document detection in heartbeat (parse PDF, extract data, create Closed Sale)
- [x] Store signed PDF to S3 and link to Closed Sale record
- [x] Send owner notification when new Pylon sale is received
- [x] Create Pylon sales heartbeat cron job (every 15 minutes) — requires deployment first
- [x] Modify Pylon handler to create draft Close Sale (pre-filled for review) instead of direct insert
- [x] Update Close Sale form to load pre-filled draft data from Pylon detection
- [x] Add 'pylon-pending-review' status to projectStatus enum
- [x] Add pendingPylonReview tRPC query to fetch unconfirmed Pylon sales
- [x] Add confirmPylonSale tRPC mutation to confirm reviewed Pylon sales
- [x] Add amber notification banner on dashboard when Pylon sales are pending review
- [x] Add 'Review Now' button that navigates to Close Sale form with pre-filled Pylon data
- [x] Add Pylon review banner inside Close Sale form indicating auto-detected source
- [x] Change submit button label to 'Confirm Sale' when reviewing a Pylon sale
- [x] Vitest coverage for pendingPylonReview and confirmPylonSale procedures
- [x] Deploy Pylon heartbeat cron job (every 15 minutes) for production (already active: oGGZkL6uQan8hrZogba2RK)
- [x] Add Dismiss/Skip action on pending Pylon review banner (skip without confirming)
- [x] Build bulk Pylon PDF import tool (upload multiple PDFs, parse all, create draft sales)
- [x] Add closed sales metrics to Overview panel (total sales, total revenue, recent closings)
