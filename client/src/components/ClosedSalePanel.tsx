...
export interface PylonSalePrefill {
  id: number;
  customerName: string;
...
interface ClosedSalePanelProps {
  ...
  prefillPylonSale?: PylonSalePrefill | null;
  onComplete?: () => void;
}
...
export default function ClosedSalePanel({ leads, prefillLead, prefillPylonSale, onComplete }: ClosedSalePanelProps) {
  const [pylonSaleId, setPylonSaleId] = useState<number | null>(prefillPylonSale?.id ?? null);
  const [formData, setFormData] = useState<ClosedSaleFormData>(() => {
    if (prefillPylonSale) {
      return {
        ...initialFormData,
        customerName: prefillPylonSale.customerName || '',
        customerEmail: prefillPylonSale.customerEmail || '',
        customerPhone: prefillPylonSale.customerPhone || '',
        installAddress: prefillPylonSale.installAddress || '',
        systemSizeDc: prefillPylonSale.systemSizeDc || '',
        panelBrand: prefillPylonSale.panelBrand || '',
        panelModel: prefillPylonSale.panelModel || '',
        panelQuantity: prefillPylonSale.panelQuantity || '',
        panelWattage: prefillPylonSale.panelWattage || '',
        inverterBrand: prefillPylonSale.inverterBrand || '',
        inverterModel: prefillPylonSale.inverterModel || '',
        inverterQuantity: prefillPylonSale.inverterQuantity || '1',
        batteryBrand: prefillPylonSale.batteryBrand || '',
        batteryModel: prefillPylonSale.batteryModel || '',
        batteryCapacityKwh: prefillPylonSale.batteryCapacityKwh || '',
        batteryQuantity: prefillPylonSale.batteryQuantity || '',
        totalContractPrice: prefillPylonSale.totalContractPrice || '',
        depositAmount: prefillPylonSale.depositAmount || '',
        depositPaid: prefillPylonSale.depositPaid || 'no',
        numberOfStcs: prefillPylonSale.numberOfStcs || '',
        stcRebateValue: prefillPylonSale.stcRebateValue || '',
        contractSignedDate: prefillPylonSale.contractSignedDate || new Date().toISOString().split('T')[0],
        phases: prefillPylonSale.phases || '1-phase',
        annualProductionEstimate: prefillPylonSale.annualProductionEstimate || '',
        dealOwner: prefillPylonSale.dealOwner || 'George Fotopoulos',
        leadSource: prefillPylonSale.leadSource || 'Pylon',
        notes: prefillPylonSale.pylonReference ? `Pylon Ref: ${prefillPylonSale.pylonReference}` : '',
      };
    }
    if (prefillLead) {
      return {
  ...
  const confirmPylonMutation = trpc.closedSales.confirmPylonSale.useMutation({
    onSuccess: () => {
      setSubmitted(true);
      setSubmitting(false);
      toast.success('Pylon sale confirmed and closed!');
      if (onComplete) onComplete();
    },
    onError: (error) => {
      setSubmitting(false);
      toast.error(`Failed to confirm sale: ${error.message}`);
    },
  });
  ...
  const handleSubmit = () => {
    ...
    // If this is a Pylon review, use confirmPylonSale instead of create
    if (pylonSaleId) {
      confirmPylonMutation.mutate({
        id: pylonSaleId,
        data: {
        ...
        <div className="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center gap-3">
          ...
          <div>
            <p className="text-amber-300 text-sm font-semibold" style={{ fontFamily: 'Urbanist' }}>
              Pylon Sale — Review & Confirm
            </p>
            <p className="text-amber-300/70 text-xs mt-0.5" style={{ fontFamily: 'General Sans' }}>
              This sale was auto-detected from a signed Pylon document. Review the pre-filled details below, make any corrections, then confirm to officially close the sale.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between mt-8 pt-6 border-t border-white/5">
          ...
          {currentStep < steps.length ? (
            ...
            >
              ...
              {submitting ? 'Saving...' : pylonSaleId ? 'Confirm Sale' : 'Close Sale'}
            </button>
          )}
        </div>
      </div>
